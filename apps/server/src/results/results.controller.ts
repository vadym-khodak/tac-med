...
export class ResultsController {
  ...
  @Post('submit-test')
  async submitTest(@Body() submitData: { fullName: string; answers: TestAnswerDto[] }) {
    ...
    if (
      ...
      !words.every(
        (word) =>
          word.length >= 3 && cyrillicPattern.test(word) && word[0] === word[0].toUpperCase(),
      )
    ) {
  ...
  @Post('calculate')
  async calculateResult(@Body() calculateData: { fullName: string; answers: TestAnswerDto[] }) {
