...
export const AdminDashboard: React.FC = () => {
  ...
  const resultsColumns = [
    ...
    ...Array.from({ length: 6 }, (_, i) => ({
      title: `Блок ${i + 1}`,
      key: `block_${i + 1}`,
      render: (record: TestResult) => `${Math.round((record.block_scores[`${i + 1}`] || 0) / 10)} з 10`,
      width: 80,
    ...
    })),
    {
      ...
      render: (record: TestResult) => {
        const total = Object.values(record.block_scores).reduce((sum, score) => sum + Math.round(score / 10), 0);
    ...
    },
    {
      ...
      render: (level: KnowledgeLevel) => (
        <Tag color={
          level === 'Максимальний' ? 'gold' :
          level === 'Високий' ? 'green' :
          level === 'Середній' ? 'blue' :
          level === 'Початковий' ? 'orange' : 'red'
        }>
          {level}
        </Tag>
      ),
  ...
  return (
    <div style={{ padding: '24px', minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <Tabs defaultActiveKey="statistics">
        <TabPane tab={<span><BarChartOutlined />Статистика</span>} key="statistics">
          {/* Statistics */}
        ...
        <TabPane tab={<span><FileTextOutlined />Результати</span>} key="results">
          <Card 
        ...
        <TabPane tab={<span><QuestionCircleOutlined />Питання</span>} key="questions">
          <Card 
                          ...
                          {record.answers.map((answer, index) => (
                            <li key={index} style={{ marginBottom: 4 }}>
                              <Text style={{ 
                                ...
                                {record.correct.includes(index) && '✓ '}{answer}
                              </Text>
                            </li>
                          ))}
                        </ul>
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <Text type="warning">
            <strong>Увага!</strong> Імпорт питань призведе до повного видалення всіх існуючих питань та їх заміни новими.
          </Text>
          <Upload
            accept=".json"
            beforeUpload={handleImportQuestions}
