...
import { Card, Button, Radio, Checkbox, Typography, Progress, Space, Modal, message, Spin } from 'antd';
...
export const TestInterface: React.FC = () => {
  ...
  const handleWindowClose = (e: BeforeUnloadEvent) => {
    if (!showInstructions) {
      e.preventDefault();
      e.returnValue = 'Тестування не завершено. Результати не будуть збережені. Ви впевнені, що хочете вийти?';
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f5', padding: '20px' }}>
      ...
      <Card style={{ marginBottom: 16, textAlign: 'center' }}>
        <Space size="large">
          <div>
            <ClockCircleOutlined style={{ fontSize: '24px', color: timeRemaining < 300 ? '#ff4d4f' : '#52c41a' }} />
            <Text style={{ fontSize: '20px', fontWeight: 'bold', marginLeft: 8, color: timeRemaining < 300 ? '#ff4d4f' : 'inherit' }}>
      ...
      <Card>
        <div style={{ marginBottom: 16 }}>
          <Text type="secondary">
            Блок {currentQuestion.block}: {BLOCK_NAMES[currentQuestion.block as keyof typeof BLOCK_NAMES]}
          </Text>
        </div>
                ...
                <Checkbox 
                  ...
                  style={{ 
                    ...
                    backgroundColor: answerSaved ? (
                      currentQuestion.correct.includes(index) ? '#f6ffed' : 
                      selectedAnswers.includes(index) && !currentQuestion.correct.includes(index) ? '#fff2f0' : 'white'
                    ) : 'white',
                    borderColor: answerSaved ? (
                      currentQuestion.correct.includes(index) ? '#52c41a' :
                      selectedAnswers.includes(index) && !currentQuestion.correct.includes(index) ? '#ff4d4f' : '#d9d9d9'
                    ) : '#d9d9d9'
                  }}
